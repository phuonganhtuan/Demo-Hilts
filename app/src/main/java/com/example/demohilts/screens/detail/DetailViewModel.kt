...
class DetailViewModel @Inject constructor(private val repo: MainRepo) : ViewModel() {
    ...
    fun getDetail(id: Int) = viewModelScope.launch {
        ...
        try {
            val detailMovie = repo.getMovieDetail(id, apiKey)
    ...
    fun getCastsAndCrews(id: Int) = viewModelScope.launch {
        castsCrews.value = BaseResults.loading(null)
        try {
            val castsCrewsRes = repo.getCastsAndCrews(id, apiKey)
    ...
    fun getSimilarMovies(id: Int, page: Int) = viewModelScope.launch {
        similars.value = BaseResults.loading(null)
        try {
            val similarsRes = repo.getSimilars(id, apiKey, page)
    ...
    fun getReviews(id: Int, page: Int) = viewModelScope.launch {
        reviews.value = BaseResults.loading(null)
        try {
            val reviewsRes = repo.getReviews(id, apiKey, page)
    ...
    fun getVideos(id: Int) = viewModelScope.launch {
        ...
        try {
            val videosRes = repo.getVideos(id, apiKey)
